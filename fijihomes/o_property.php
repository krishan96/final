...
<style type="text/css">
  ...
  <br>
	...
	<?php
      $dal = new DAL();
      $res = $dal->propertydetails($_SESSION['filename']);
      $row=$res;
	  if($res){
...
<th style=" text-align: center;">
	<?php
      ...
      echo "Area: ".$row[0]->Area."</br>";
       ...
       ?><p style="color:red;">You currently do not have any property registered.<br> Feel free to contact us.<p><?php
     }?>
	 </th>
			</tr>
		</tfoot>
	...
	<table  class="data-table tab2">
		...
		<tfoot>
			<tr>
				<th style=" text-align: center;">
				 <?php
            $res1= $dal->allocateagent($_SESSION['filename']);
            if($res1){
                $row1 = $res1;
		...
		<tfoot>
			<tr>
				<th style=" text-align: center;">
					   <?php
          if($res){
            $res2 =  $dal->tenantdetails($row[0]->Plot);
            if($res2){
                $row2= $res2;
